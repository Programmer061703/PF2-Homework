...
void readFile ( string filename, Pinball Temp[MAX]){
    // This code utalizes the power of UNGA BUNGA to acheive its goals. How it operates is a mystery 
...
// Switch statment is responsible for handeling the user input and proccessing them 
switch (selection)
{
...
case(2):
    cout << "Please input the (Machine name, Year Produced, Manufacturer, Funrating, Themes) all seperated by a comma  "<<endl;
...
cout <<"Please input the specific year you wish to find"<<endl;
cin >> year; 
for (int i = 0; i < MAX; i++){
